...
def 依存関係チェック・インストール():
    ...
    for パッケージ名 in 必須パッケージ:
        ...
        if spec is None:
            ...
            print(f"  ❌ {パッケージ名} - 未インストール")
        else:
            print(f"  ✅ {パッケージ名} - インストール済み")
    
    if 未インストール:
        print(f"\n⚠️  {len(未インストール)}個の必須パッケージが不足しています")
        print(f"📦 自動インストールを実行します...")
...
def オプションライブラリチェック():
    ...
    for パッケージ名 in オプションパッケージ:
        ...
        if spec is None:
            未インストール.append(パッケージ名)
            print(f"  ⚠️  {パッケージ名} - 未インストール（標準版で動作）")
        else:
            print(f"  ✅ {パッケージ名} - インストール済み（高速化機能利用可能）")
    
    if 未インストール:
        print(f"\n💡 高速化のため、以下のコマンドでオプションライブラリをインストールできます:")
        print(f"pip install {' '.join(未インストール)}")
        print(f"\n📝 機能差:")
...
def 仮想環境チェック():
    ...
    if os.path.exists(venv_path):
        ...
        if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
            ...
            print(f"⚠️  仮想環境が非アクティブです")
            print(f"💡 以下のコマンドで仮想環境をアクティベートしてください:")
            print(f"source .venv/bin/activate")
            print(f"python3 core/scopus解析.py")
            return False
    else:
        print(f"📦 仮想環境が見つかりません - グローバル環境で実行")
        return True
...
def メインスクリプト起動():
    ...
    if os.path.exists(core_script):
        ...
        print(f"❌ メインスクリプトが見つかりません: {core_script}")

def main():
    """メイン処理"""
    print("🎯 Scopus文献可視化システム - スタートアップ")
    print("=" * 50)
