...
def main() -> None:
    ...
    print(f"[INFO] 検出されたCSVファイル: {len(csvs)}件")
    for csv_file in csvs:
        print(f"  - {os.path.basename(csv_file)}")

    if not csvs:
        print("[NG] 結合対象のCSVファイルが見つかりません")
        print("[HINT] 使用方法:")
        print("  1. 作業フォルダにScopus CSVファイルを配置")
        print("  2. その作業フォルダ内でこのツールを実行")
        return

    print(f"[DATA] CSVファイルを結合中...")
    ...
    print(f"[OK] 結合完了:")
    print(f"  [DIR] 入力: {len(csvs)}ファイル")
    print(f"  [DATA] 元データ: {original_count:,}行")
    print(f"  [PROC] 重複除去後: {deduplicated_count:,}行")
    print(f"  💾 出力: {OUT_NAME}")
    
    if original_count != deduplicated_count:
        print(f"  [WARN]  {original_count - deduplicated_count:,}行の重複を除去")
