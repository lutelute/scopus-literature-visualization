    ...
    try:
        ...
        if 結果.returncode == 0:
            ...
            if 結果.stdout:
                # 成功メッセージと統計のみ表示
                出力行 = 結果.stdout.split('\n')
                重要行 = [行 for 行 in 出力行 if any(マーク in 行 for マーク in ['[OK]', '[DATA]', '[DIR]', '[CHART]', '[DONE]', 'PDF取得完了', '新規PDF', '総PDF'])]
        ...
        if 選択 == '0':
            ...
            for スクリプト, 説明 in pdf取得方法:
                if pdf取得実行(スクリプト, 説明):
                    ...
                    print(f"[CHART] 累積新規PDF: {新規pdf数}件 (総計: {現在pdf数}件)")
                else:
                    print(f"[WARN] {説明}でエラーが発生しました。")
            ...
            print(f"\n[DONE] 全PDF取得処理完了!")
            print(f"[DATA] 成功率: {成功数}/{len(pdf取得方法)} 方法")
            print(f"📥 新規PDF取得: {総新規pdf数}件")
            print(f"[DIR] 総PDF数: {最終pdf数}件")
            break
            ...
            if pdf取得実行(スクリプト, 説明):
                ...
                print(f"\n[CHART] 新規PDF取得: {新規pdf数}件")
                print(f"[DIR] 総PDF数: {最終pdf数}件")
            break
        
        else:
            print("[NG] 無効な選択です。0-3またはqを入力してください。")
